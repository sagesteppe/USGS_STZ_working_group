...
Three data sets, which may be of interest, use downscaling approaches to generate high resolution surfaces, (WorldClim 2.0, CHELSA, and climateNA), and two of these data sets, ClimateNA (to ~30m resolution) and CHELSA (~ 1 km resolution), offer already computed bioclim variables for a range of time periods for download (Brun et al. 2022, Fick & Hijman 2017, Mahony et al. 2022, Wang et al. 2016). 

Models of seed zones can be fit under both current conditions, and at the time points offered by the above models. 
For users unsure of which data products to use, i.e. which global climate model for a set of SSPs or RCPs, the same statistical model can be fit to a variety of forecasts and a weighted mean (based on an evaluation metric, e.g. r^2^) and the predictions ensembled to create a single prediction, as well as estimates of uncertainty (see Dormann et al. 2018). 
